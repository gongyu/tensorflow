...
bool SegmentTest::GetGraphDef(TF_Graph* graph,
  ...
  if (ret)
    ret = graph_def->ParseFromArray(buffer->data, buffer->length);
...
TEST_F(SegmentTest, AvoidCycle) {
  ...
  // add2 is not a TensorOpt candidate so add0/add3 cannot be formed as a
  // subgraph
  //
  //           feed
  //         //    ||
...
TEST_F(SegmentTest, Multiple) {
  ...
  // add5 is not a TensorOpt candidate so two subgraphs should be formed
  //
  //                feed
  //         //      ||     ||
  //       add0    add1      add7
...
TEST_F(SegmentTest, BigIfElse) {
  ...
  // add2 is not a TensorOpt candidate
  //
  //           feed
  //            ||
  //           add0
