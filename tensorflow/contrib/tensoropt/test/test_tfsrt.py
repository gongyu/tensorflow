...
def user(run_graph=execute_graph, run_calibration=execute_calibration):
  """Example function that converts a graph to TF-TensorOpt graph."""

  inp_dims = (100, 24, 24, 2)
  dummy_input = np.random.random_sample(inp_dims)
  orig_graph = get_simple_graph_def()  # use a frozen graph for inference
  # Get optimized graph
  tensoropt_graph = topt.create_inference_graph(
      input_graph_def=orig_graph,
      outputs=["output"],
      max_batch_size=inp_dims[0],
      max_workspace_size_bytes=1 << 25,
      precision_mode="FP32",  # TensorOpt Engine precision "FP32","FP16" or "INT8"
      minimum_segment_size=2  # minimum number of nodes in an engine
  )
  o1 = run_graph(orig_graph, dummy_input)
  o2 = run_graph(tensoropt_graph, dummy_input)
  o3 = run_graph(tensoropt_graph, dummy_input)
  assert np.allclose(o1, o2)
  assert np.allclose(o3, o2)  # sanity check
  print("Pass")
...
if "__main__" in __name__:
  P = argparse.ArgumentParser(
      prog="tftopt_test",
      description="Example utilization of TensorFlow-TensorOpt integration")
  P.add_argument(
      "--automatic",
      "-a",
      action="store_true",
      help="Do TensorOpt conversion automatically",
      default=False)
