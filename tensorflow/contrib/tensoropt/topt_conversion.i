...
%ignoreall
%unignore tensorflow;
%unignore topt_convert;

%{

std::pair<string, string> topt_convert(
  ...
  tensorflow::Status conversion_status =
      tensorflow::tensoropt::convert::ConvertGraphDefToTensorOpt(
          graph_def, output_names, &outGraph, minimum_segment_size);
  ...
  return std::pair<string, string>{"9;TensorOpt is not enabled!", ""};
#endif  // defined(TENSORFLOW_USE_SYCL) && TF_SYCL_USE_TENSOROPT
}
%}

std::pair<string, string> topt_convert(string graph_def_string,
                                      std::vector<string> output_names,
                                      size_t max_batch_size,
                                      size_t max_workspace_size_bytes,
